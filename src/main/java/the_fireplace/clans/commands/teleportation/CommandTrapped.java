...
public class CommandTrapped extends ClanSubCommand {
	...
	public void run(@Nullable MinecraftServer server, EntityPlayerMP player, String[] args) {
		...
		} else if(chunkOwner != null && !chunkOwner.getMembers().containsKey(player.getUniqueID()) && (!RaidingParties.hasActiveRaid(chunkOwner) || !RaidingParties.getActiveRaid(chunkOwner).getMembers().contains(player))) {
