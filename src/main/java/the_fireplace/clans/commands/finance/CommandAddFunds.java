		...
		if(!selectedClan.isServer()) {
			...
			if(Clans.getPaymentHandler().deductAmount(amount, sender.getUniqueID())) {
				if(Clans.getPaymentHandler().addAmount(amount, selectedClan.getId())) {
					sender.sendMessage(TranslationUtil.getTranslation(sender.getUniqueID(), "commands.clan.addfunds.success", Clans.getPaymentHandler().getFormattedCurrency(amount), selectedClan.getName()).setStyle(TextStyles.GREEN));
					selectedClan.messageAllOnline(sender, TextStyles.GREEN, "commands.clan.addfunds.added", sender.getDisplayNameString(), Clans.getPaymentHandler().getFormattedCurrency(amount), selectedClan.getName());
				} else {
