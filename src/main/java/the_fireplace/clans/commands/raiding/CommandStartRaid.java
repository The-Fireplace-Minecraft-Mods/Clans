				...
				if(clanPlayers.size() >= raid.getAttackerCount() - Clans.getConfig().getMaxRaidersOffset()) {
					if(!RaidingParties.hasActiveRaid(raid.getTarget())) {
						if(!RaidingParties.isPreparingRaid(raid.getTarget())) {
							double raidCost = FormulaParser.eval(Clans.getConfig().getStartRaidCostFormula(), raid.getTarget(), raid, 0);
							...
							if (Clans.getPaymentHandler().deductAmount(raidCost, sender.getUniqueID())) {
								...
								sender.sendMessage(TranslationUtil.getTranslation(sender.getUniqueID(), "commands.raid.start.insufficient_funds", raid.getTarget().getName(), Clans.getPaymentHandler().getFormattedCurrency(raidCost)).setStyle(TextStyles.RED));
						} else
