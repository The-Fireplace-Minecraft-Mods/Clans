				...
				if(!target.getMembers().containsKey(sender.getUniqueID())) {
					if (!RaidingParties.getRaidedClans().contains(target)) {
						if(!target.isShielded()) {
							if (!target.getOnlineSurvivalMembers().isEmpty() && target.getOnlineSurvivalMembers().size() + ClansHelper.getConfig().getMaxRaidersOffset() > 0) {
						...
						if(target.getOnlineSurvivalMembers().size() + ClansHelper.getConfig().getMaxRaidersOffset() > raid.getAttackerCount()) {
							...
							sender.sendMessage(TranslationUtil.getTranslation(sender.getUniqueID(), "commands.raid.join.limit", target.getName(), raid.getAttackerCount(), target.getOnlineMembers().size() + ClansHelper.getConfig().getMaxRaidersOffset()).setStyle(TextStyles.RED));
					}
				} else
