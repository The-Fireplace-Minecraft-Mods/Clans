...
public class CommandGrantAccess extends ClanSubCommand {
	@Override
	...
	public void run(MinecraftServer server, EntityPlayerMP sender, String[] args) throws CommandException {
		...
		if(lookRay == null || lookRay.typeOfHit != RayTraceResult.Type.BLOCK) {
			sender.sendMessage(TranslationUtil.getTranslation(sender.getUniqueID(), "commands.clan.grantaccess.not_block").setStyle(TextStyles.RED));
			return;
		}
		...
		if(selectedClan.isLocked(targetBlockPos) && !selectedClan.isLockOwner(targetBlockPos, sender.getUniqueID()) && !selectedClan.hasPerm("lockadmin", sender.getUniqueID())) {
			sender.sendMessage(TranslationUtil.getTranslation(sender.getUniqueID(), "commands.clan.grantaccess.locked", Objects.requireNonNull(server.getPlayerProfileCache().getProfileByUUID(Objects.requireNonNull(selectedClan.getLockOwner(targetBlockPos)))).getName()).setStyle(TextStyles.RED));
			return;
		}
		...
		if(selectedClan.isLocked(targetBlockPos)) {
			selectedClan.addLockOverride(targetBlockPos, targetPlayerId, true);
			for(BlockPos pos: MultiblockUtil.getLockingConnectedPositions(sender.world, targetBlockPos, state))
				selectedClan.addLockOverride(pos, targetPlayerId, true);
