...
public final class NewRaidRestoreDatabase {
	...
	public static NewChunkRestoreData popChunkRestoreData(int dim, Chunk c) {
		NewChunkRestoreData d = getInstance().raidedChunks.remove(new ChunkPosition(c.x, c.z, dim));
		if(d != null)
			isChanged = true;
		return d;
	}
