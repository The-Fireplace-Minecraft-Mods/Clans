					...
					if (breakingPlayer instanceof EntityPlayerMP) {
						...
						if (!ClanCache.isClaimAdmin((EntityPlayerMP) breakingPlayer) && (playerClans.isEmpty() || !playerClans.contains(chunkClan)) && !isRaided) {
							...
							if (targetState.getBlock().hasTileEntity(targetState)) {
								...
								NewRaidRestoreDatabase.addRestoreBlock(c.getWorld().provider.getDimension(), c, event.getPos(), BlockSerializeUtil.blockToString(targetState));
						}
					}
					return;
				} else {
			...
			if (chunkOwner != null) {
				...
				if (chunkClan != null) {
					...
					if (interactingPlayer instanceof EntityPlayerMP) {
						...
						IBlockState targetState = event.getWorld().getBlockState(event.getPos());
						if (!ClanCache.isClaimAdmin((EntityPlayerMP) interactingPlayer) && (playerClan.isEmpty() || !playerClan.contains(chunkClan)) && (!RaidingParties.isRaidedBy(chunkClan, interactingPlayer) || !(targetState.getBlock() instanceof BlockDoor || targetState.getBlock() instanceof BlockTrapDoor || targetState.getBlock() instanceof BlockFenceGate || targetState.getBlock() instanceof BlockTNT))) {
			...
			for (BlockPos pos : event.getAffectedBlocks()) {
				...
				if (chunkClan != null) {
					...
					if (RaidingParties.hasActiveRaid(chunkClan) && !targetState.getBlock().hasTileEntity(targetState)) {
						NewRaidRestoreDatabase.addRestoreBlock(c.getWorld().provider.getDimension(), c, pos, BlockSerializeUtil.blockToString(targetState));
					} else {
