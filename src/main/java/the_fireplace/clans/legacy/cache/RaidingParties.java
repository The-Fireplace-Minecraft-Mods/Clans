...
public final class RaidingParties {
	...
	public static Raid getInactiveRaid(String clanname){
		return inactiveRaids.get(ClanNameCache.getClanByName(clanname));
	}
	...
	public static void initRaid(Clan raidTarget){
		...
		raidTarget.messageAllOnline(true, TextStyles.GREEN, "clans.raid.init.defender", inactiveRaids.get(raidTarget).getAttackerCount(), raidTarget.getName(), ClansModContainer.getConfig().getRaidBufferTime());
		for(UUID raiderId: getInactiveRaids().get(raidTarget).getAttackers()) {
			...
			raiderEntity.sendStatusMessage(TranslationUtil.getTranslation(raiderId, "clans.raid.init.attacker", inactiveRaids.get(raidTarget).getAttackerCount(), raidTarget.getName(), ClansModContainer.getConfig().getRaidBufferTime()).setStyle(TextStyles.GREEN), true);
			if(ClansModContainer.getConfig().isTeleportToRaidStart() && raidTarget.hasHome() && raidTarget.getHome() != null) {
