		...
		if(!isCoolingDown || sender.isCreative()) {
			if (!selectedClan.hasHome() || home == null)
				...
				if(ClansModContainer.getConfig().getClanHomeWarmupTime() > 0 && !sender.isCreative()) {
					sender.sendMessage(TranslationUtil.getTranslation(sender.getUniqueID(), "commands.clan.home.warmup", selectedClan.getName(), ClansModContainer.getConfig().getClanHomeWarmupTime()).setStyle(TextStyles.GREEN));
