	...
	public void run(MinecraftServer server, EntityPlayerMP sender, String[] args) {
		if(selectedClan.getMembers().get(sender.getUniqueID()).equals(EnumRank.LEADER)) {
			if(!selectedClan.isServer()) {
				if(ClansModContainer.getConfig().getDisbandFeeFormula().isEmpty() || ClansModContainer.getPaymentHandler().deductAmount(selectedClan.getDisbandCost(), selectedClan.getId())) {
					...
					sender.sendMessage(TranslationUtil.getTranslation(sender, "commands.clan.disband.insufficient_funds", selectedClan.getName(), selectedClan.getDisbandCost()).setStyle(TextStyles.RED));
			} else
