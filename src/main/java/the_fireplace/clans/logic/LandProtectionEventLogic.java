...
public class LandProtectionEventLogic {
    public static boolean onBlockBroken(World world, BlockPos pos, EntityPlayer breaker) {
        if(!world.isRemote) {
            ...
            if (chunkOwner != null) {
                ...
                if (chunkClan != null) {
                    if (breaker instanceof EntityPlayerMP) {
                        ...
                        if (!ClanCache.isClaimAdmin((EntityPlayerMP) breaker) && (playerClans.isEmpty() || !playerClans.contains(chunkClan)) && !isRaided && !Clans.getMinecraftHelper().isAllowedNonPlayerEntity(breaker)) {
            ...
            if (Clans.getConfig().isProtectWilderness() && (Clans.getConfig().getMinWildernessY() < 0 ? pos.getY() >= world.getSeaLevel() : pos.getY() >= Clans.getConfig().getMinWildernessY()) && !Clans.getMinecraftHelper().isAllowedNonPlayerEntity(breaker) && (!(breaker instanceof EntityPlayerMP) || (!ClanCache.isClaimAdmin((EntityPlayerMP) breaker) && !PermissionManager.hasPermission((EntityPlayerMP)breaker, PermissionManager.PROTECTION_PREFIX+"break.protected_wilderness")))) {
            ...
            if (placer instanceof EntityPlayerMP) {
                if (chunkOwner != null) {
                    ...
                    if (chunkClan != null) {
                        ...
                        if ((!ClanCache.isClaimAdmin((EntityPlayerMP) placer) && (playerClans.isEmpty() || (!playerClans.contains(chunkClan) && !RaidingParties.isRaidedBy(chunkClan, placer)))) && !Clans.getMinecraftHelper().isAllowedNonPlayerEntity(placer)) {
            ...
            if (chunkOwner != null) {
                ...
                if (chunkClan != null) {
                    if (player instanceof EntityPlayerMP) {
                        ...
                        IBlockState targetState = world.getBlockState(pos);
                        if (!ClanCache.isClaimAdmin((EntityPlayerMP) player) && (playerClan.isEmpty() || !playerClan.contains(chunkClan)) && (!RaidingParties.isRaidedBy(chunkClan, player) || targetState.getBlock() instanceof BlockContainer || targetState.getBlock() instanceof BlockDragonEgg)) {
    ...
    public static boolean onLivingDamage(Entity target, @Nullable Entity attacker) {
        if(!target.getEntityWorld().isRemote) {
            ...
            if(attacker instanceof EntityPlayerMP && ClanCache.isClaimAdmin((EntityPlayerMP) attacker) || chunkClan == null)
                return false;
            if(target instanceof EntityPlayer || (target instanceof EntityTameable && ((EntityTameable) target).getOwnerId() != null)) {
                ...
                if(attacker instanceof EntityPlayer || (attacker instanceof EntityTameable && ((EntityTameable) attacker).getOwnerId() != null)) {
                    ...
                    if(sourceClans.contains(chunkClan) || RaidingParties.hasActiveRaid(chunkClan) || Clans.getMinecraftHelper().isAllowedNonPlayerEntity(attacker))
    ...
    public static boolean onEntitySpawn(World world, Entity entity) {
        if(Clans.getConfig().isPreventMobsOnClaims() && !world.isRemote && entity instanceof IMob) {
            if(ClaimDataManager.getChunkClan(entity.chunkCoordX, entity.chunkCoordZ, entity.dimension) != null)
                return true;
        }
        return false;
    }
