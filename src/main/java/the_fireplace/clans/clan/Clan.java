...
public class Clan implements ThreadedSaveable, JsonWritable {
    ...
    private long rentTimestamp = System.currentTimeMillis() + ClansModContainer.getConfig().getChargeRentDays() * 1000L * 60L * 60L * 24L, upkeepTimestamp = System.currentTimeMillis() + ClansModContainer.getConfig().getClanUpkeepDays() * 1000L * 60L * 60L * 24L;
    private int color;
    ...
    public Clan(String clanName, UUID leader) {
        ...
        if (ClansModContainer.getPaymentHandler().getBalance(clanId) > 0)
            ClansModContainer.getPaymentHandler().deductAmount(ClansModContainer.getPaymentHandler().getBalance(clanId), clanId);

        ClansModContainer.getPaymentHandler().addAmount(ClansModContainer.getConfig().getFormClanBankAmount(), clanId);
        ClanMemberCache.cachePlayerClan(leader, this);
        if(!ClansModContainer.getConfig().isAllowMultiClanMembership())
            for(Clan clan: ClanDatabase.getClans())
                InvitedPlayers.removeInvite(leader, clan.getId());
        rent = Math.min(FormulaParser.eval(ClansModContainer.getConfig().getMaxRentFormula(), this, 0)/ ClansModContainer.getConfig().getChargeRentDays(), FormulaParser.eval(ClansModContainer.getConfig().getClanUpkeepCostFormula(), this, 0)/ ClansModContainer.getConfig().getClanUpkeepDays())* ClansModContainer.getConfig().getChargeRentDays();
    ...
    public double payLeaders(double totalAmount) {
        ...
        for(UUID leader: leaders)
            ClansModContainer.getPaymentHandler().addAmount(totalAmount, leader);
        return 0;
    }
    ...
    public void setName(String clanName) {
        ClanNameCache.removeName(this.clanName);
        this.clanName = clanName;
        ClanNameCache.addName(this);
        markChanged();
    }
    ...
    public void setBanner(String clanBanner) {
        ClanBanners.uncacheBanner(this.clanBanner);
        ClanBanners.cacheBanner(clanBanner);
    ...
    public void setHome(BlockPos pos, int dimension) {
        ...
        markChanged();
        ClanHomes.setClanHome(this, pos);
    }
    ...
    public long getMaxClaimCount() {
        ...
        return ClansModContainer.getConfig().isMultiplyMaxClaimsByPlayers()
            ? getMemberCount() * ClansModContainer.getConfig().getMaxClaims()
    ...
    public double getClaimCost(long currentClaimCount) {
        ...
        return currentClaimCount < ClansModContainer.getConfig().getReducedCostClaimCount()
                ...
                : FormulaParser.eval(ClansModContainer.getConfig().getClaimChunkCostFormula(), this, 0);
    }
    ...
    public void addMember(UUID player) {
        ...
        ClansModContainer.getDynmapCompat().refreshTooltip(this);
        markChanged();
    }
    ...
    public void updateNextRentTimestamp() {
        this.rentTimestamp = System.currentTimeMillis() + ClansModContainer.getConfig().getChargeRentDays() * 1000L * 60L * 60L * 24L;
        markChanged();
    }
    ...
    public void updateNextUpkeepTimestamp() {
        this.upkeepTimestamp = System.currentTimeMillis() + ClansModContainer.getConfig().getClanUpkeepDays() * 1000L * 60L * 60L * 24L;
        markChanged();
    }
    ...
    private void decreaseMultiplier() {
        setMultiplier(ClansModContainer.getConfig().getDecreasedMultiplierFormula());
    }

    private void increaseMultiplier() {
        setMultiplier(ClansModContainer.getConfig().getIncreasedMultiplierFormula());
    }
    ...
    public double getDisbandCost() {
        return FormulaParser.eval(ClansModContainer.getConfig().getDisbandFeeFormula(), this, 1.0);
    }
    ...
    public void refundClaim() {
        ClansModContainer.getPaymentHandler().addAmount(getClaimCost(), getId());
    }

    public boolean payForClaim() {
        return ClansModContainer.getPaymentHandler().deductAmount(getClaimCost(), getId());
    }
    ...
    public void disband(MinecraftServer server, @Nullable ICommandSender sender, String disbandMessageTranslationKey, Object... translationArgs) {
        ...
        ClansModContainer.getDynmapCompat().clearAllClanMarkers(this);
    ...
    private void distributeFunds(MinecraftServer server, ICommandSender sender, String disbandMessageTranslationKey, Object... translationArgs) {
        double distFunds = ClansModContainer.getPaymentHandler().getBalance(this.getId());
        distFunds += FormulaParser.eval(ClansModContainer.getConfig().getClaimChunkCostFormula(), this, 0) * this.getClaimCount();
        ...
        for (UUID member : this.getMembers().keySet()) {
            ClansModContainer.getPaymentHandler().addAmount(distFunds, member);
            PlayerClanSettings.updateDefaultClanIfNeeded(member, getId());
        ...
        ClansModContainer.getPaymentHandler().deductAmount(ClansModContainer.getPaymentHandler().getBalance(this.getId()), this.getId());
    }
