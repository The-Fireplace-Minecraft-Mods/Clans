...
import dev.the_fireplace.clans.legacy.player.PlayerRaidStats;
...
public class Raid
{
    ...
    public Raid(ServerPlayerEntity starter, UUID targetClan) {
    ...
    public boolean checkRaidEndTimer() {
        if (remainingSeconds == ClansModContainer.getConfig().getRemainingTimeToGlow() * 60) {
            for (UUID member : defenders.keySet()) {
                ServerPlayerEntity d2 = ClansModContainer.getMinecraftHelper().getServer().getPlayerList().getPlayerByUUID(member);
            ...
            for (UUID member : getAttackers()) {
                ServerPlayerEntity m2 = ClansModContainer.getMinecraftHelper().getServer().getPlayerList().getPlayerByUUID(member);
                //noinspection ConstantConditions
                if (m2 != null) {
        ...
        if (remainingSeconds-- <= ClansModContainer.getConfig().getRemainingTimeToGlow() * 60) {
            for (UUID defender : defenders.keySet()) {
                ServerPlayerEntity d2 = ClansModContainer.getMinecraftHelper().getServer().getPlayerList().getPlayerByUUID(defender);
                //noinspection ConstantConditions
                if (d2 != null) {
    ...
    public void setDefenders(Iterable<ServerPlayerEntity> defenders) {
        for (ServerPlayerEntity defender : defenders) {
