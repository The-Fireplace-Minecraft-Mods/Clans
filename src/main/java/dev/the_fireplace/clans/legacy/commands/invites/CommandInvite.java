...
public class CommandInvite extends ClanSubCommand
{
    ...
    public static void invitePlayer(MinecraftServer server, ServerPlayerEntity sender, String inviteTarget, UUID invitingClan) throws PlayerNotFoundException {
                ...
                if (!PlayerClans.getClansPlayerIsIn(target.getId()).contains(invitingClan)) {
                    if (InvitedPlayers.isBlockingAllInvites(target.getId())) {
                        ...
                        if (ArrayUtils.contains(server.getOnlinePlayerProfiles(), target)) {
                            ServerPlayerEntity targetEntity = server.getPlayerList().getPlayerByUUID(target.getId());
    ...
    public static void revokeInvite(MinecraftServer server, ServerPlayerEntity sender, String inviteTarget, UUID revokingClan) throws PlayerNotFoundException {
        GameProfile target = null;
        try {
            target = server.getPlayerProfileCache().getGameProfileForUsername(inviteTarget);
        } catch (Exception ignored) {
