...
public abstract class ClanSubCommand extends CommandBase
{
    ...
    public final void execute(@Nullable MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (!allowConsoleUsage() && !(sender instanceof ServerPlayerEntity)) {
        ...
        if (hasClanName) {
            ...
            if (sender instanceof ServerPlayerEntity) {
                Collection<UUID> playerClans = PlayerClans.getClansPlayerIsIn(((ServerPlayerEntity) sender).getUniqueID());
                if (playerClan != null && !playerClans.contains(playerClan)) {
                    sender.sendMessage(TranslationUtil.getTranslation(((ServerPlayerEntity) sender).getUniqueID(), "commands.clan.common.player_not_in_clan", sender.getName(), ClanNames.get(playerClan).getName()).setStyle(TextStyles.RED));
                    return;
                }
            }
        ...
        if (checkPermission(server, sender)) {
            if (sender instanceof ServerPlayerEntity) {
                run(server, (ServerPlayerEntity) sender, args2);
            } else {
    ...
    protected void run(MinecraftServer server, ServerPlayerEntity sender, String[] args) throws CommandException {
