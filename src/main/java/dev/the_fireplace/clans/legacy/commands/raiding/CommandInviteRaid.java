...
public class CommandInviteRaid extends RaidSubCommand
{
    ...
    public void run(MinecraftServer server, ServerPlayerEntity sender, String[] args) throws CommandException {
        if (RaidingParties.getRaidingPlayers().contains(sender.getUniqueID())) {
            ...
            if (raid != null) {
                ...
                ServerPlayerEntity target = targetProfile != null ? server.getPlayerList().getPlayerByUUID(targetProfile.getId()) : null;
                if (target != null) {
                    Collection<ServerPlayerEntity> clanPlayers = ClanMembers.get(raid.getTarget()).getOnlineMembers();
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
        ...
        if (sender instanceof ServerPlayerEntity) {
            Raid r = RaidingParties.getRaid((ServerPlayerEntity) sender);
            if (r != null) {
