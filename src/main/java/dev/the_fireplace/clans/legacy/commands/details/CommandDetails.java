...
import dev.the_fireplace.clans.legacy.util.TimeUtils;
...
public class CommandDetails extends ClanSubCommand
{
        ...
        if (args.length == 0) {
            if (selectedClan == null) {
                if (sender instanceof ServerPlayerEntity) {
                    sender.sendMessage(TranslationUtil.getTranslation(((ServerPlayerEntity) sender).getUniqueID(), "commands.clan.details.noclan", TranslationUtil.getTranslation(((ServerPlayerEntity) sender).getUniqueID(), getUsage(sender)).getFormattedText()).setStyle(TextStyles.RED));
                } else {
    ...
    private void showDetails(MinecraftServer server, ICommandSender sender, UUID clan) throws CommandException {
        ...
        UUID senderId = sender instanceof ServerPlayerEntity ? ((ServerPlayerEntity) sender).getUniqueID() : null;
