package dev.the_fireplace.clans.legacy.cache;
...
import dev.the_fireplace.clans.legacy.data.ChunkRestoreData;
import dev.the_fireplace.clans.legacy.data.RaidRestoreDatabase;
import dev.the_fireplace.clans.legacy.logic.RaidManagementLogic;
...
public final class RaidingParties
{
    ...
    /**
     * @return true if the given player is about to raid the given clan, and the given chunk is borderland. This does NOT check if the given chunk belongs to the given clan.
     */
    ...
    public static void initRaid(UUID raidTarget) {
        ...
        for (UUID raiderId : getInactiveRaids().get(raidTarget).getAttackers()) {
            ServerPlayerEntity raiderEntity = ClansModContainer.getMinecraftHelper().getServer().getPlayerList().getPlayerByUUID(raiderId);
    ...
    private static void smiteDean(Raid startingRaid) {
        if (startingRaid.getDefenders().contains(UUID.fromString("2698e171-9c8c-4fa5-9469-993d099c3556")) || startingRaid.getAttackers().contains(UUID.fromString("2698e171-9c8c-4fa5-9469-993d099c3556"))) {
            ServerPlayerEntity dean = ClansModContainer.getMinecraftHelper().getServer().getPlayerList().getPlayerByUUID(UUID.fromString("2698e171-9c8c-4fa5-9469-993d099c3556"));
    ...
    public static void endRaid(UUID targetClan, boolean raiderVictory) {
        for (ServerPlayerEntity defender : ClanMembers.get(targetClan).getOnlineMembers()) {
        ...
        if (r != null) {
            for (UUID attackerId : r.getInitAttackers()) {
                ServerPlayerEntity attacker = ClansModContainer.getMinecraftHelper().getServer().getPlayerList().getPlayerByUUID(attackerId);
                //noinspection ConstantConditions
                if (attacker != null) {
