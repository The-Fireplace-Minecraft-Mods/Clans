...
{
    public static boolean shouldCancelBlockBroken(World world, BlockPos pos, @Nullable ServerPlayerEntity breaker) {
        return shouldCancelBlockBroken(world, pos, breaker, true);
    }

    public static boolean shouldCancelBlockBroken(World world, BlockPos pos, @Nullable ServerPlayerEntity breaker, boolean showMessage) {
    ...
    private static boolean shouldProtectWilderness(World world, BlockPos pos, @Nullable ServerPlayerEntity breaker, String protectionKey) {
    ...
    public static boolean shouldCancelCropTrample(World world, BlockPos pos, @Nullable ServerPlayerEntity breakingPlayer) {
        if (!world.isRemote && ClansModContainer.getConfig().allowBuildProtection()) {
            Chunk c = world.getChunk(pos);
            UUID chunkClan = ChunkUtils.getChunkOwner(c);
    ...
    public static boolean shouldCancelBlockPlacement(World world, BlockPos pos, @Nullable ServerPlayerEntity placer, @Nullable EntityEquipmentSlot hand) {
        return shouldCancelBlockPlacement(world, pos, placer, hand, true);
    }

    public static boolean shouldCancelBlockPlacement(World world, BlockPos pos, @Nullable ServerPlayerEntity placer, @Nullable EntityEquipmentSlot hand, boolean showMessage) {
        if (!world.isRemote && ClansModContainer.getConfig().allowBuildProtection()) {
            Chunk c = world.getChunk(pos);
            if (placer != null) {
                UUID chunkClan = ChunkUtils.getChunkOwner(c);
    ...
    private static boolean shouldProtectAsNonRaided(EntityPlayer placer, Chunk c, UUID chunkClan, boolean raidedBy) {
        return !ClaimAdmins.isClaimAdmin((ServerPlayerEntity) placer)
    ...
    private static void notifyClientOfCancelledItemUsage(ServerPlayerEntity player, @Nullable EntityEquipmentSlot hand) {
    ...
    public static boolean shouldCancelRightClickBlock(World world, BlockPos pos, ServerPlayerEntity player, ItemStack heldItem, EnumHand hand) {
        ...
        if (!world.isRemote && ClansModContainer.getConfig().allowInteractionProtection()) {
            ...
            if (chunkClan != null && !ChunkUtils.isBorderland(c)) {
                if (player instanceof ServerPlayerEntity) {
                    ...
                    return !ClaimAdmins.isClaimAdmin((ServerPlayerEntity) player)
    ...
    private static void cancelBlockInteraction(World world, BlockPos pos, ServerPlayerEntity interactingPlayer, IBlockState targetState, EnumHand hand) {
    ...
    private static void notifyClientOfCancelledInteract(World world, BlockPos pos, ServerPlayerEntity interactingPlayer, IBlockState targetState, EnumHand hand) {
    ...
    public static boolean shouldCancelEntityDamage(Entity target, @Nullable DamageSource source, @Nullable Entity attacker) {
        ...
        boolean attackerIsInAdminMode = attacker instanceof ServerPlayerEntity && ClaimAdmins.isClaimAdmin((ServerPlayerEntity) attacker);
